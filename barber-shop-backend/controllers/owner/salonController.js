  ...
  try {
    ...
    if (!salon.isApproved) {
      return res.status(403).json(
        formatErrorResponse(messages.SALON.SALON_NOT_APPROVED, 403)
      );
    }
    ...
    res.json(
      formatSuccessResponse(
        {
          ...salon,
          estimatedWaitTime: queue?.averageWaitTime || 0,
        },
        messages.GENERIC.RETRIEVED
      )
    );
...
// ===================================================
exports.searchNearbySalons = async (req, res) => {

  try {

    const { latitude, longitude, radius = 10000, city } = req.query;
    ...
    const salons = await Salon.find(query)
      .select('name address city phone photos logo averageRating totalReviews acceptsOnlinePayment')
      .limit(20)
      .lean();
...
// ===================================================
exports.searchSalonsByName = async (req, res) => {
  ...
  try {
    ...
    if (!q || q.trim().length < 2) {
      return res.status(400).json(
        formatErrorResponse('Search query must be at least 2 characters', 400)
      );
    }
