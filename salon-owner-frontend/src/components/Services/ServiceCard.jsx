...
const ServiceCard = ({ service, onEdit, onDelete, onToggle, loading = false }) => {
  ...
  const confirmDeactivate = () => { setShowConfirm(false); onToggle(service._id || service.id, false); };
  const confirmDelete     = () => { setShowDelete(false);  onDelete(service._id || service.id); };
  ...
  return (
    <>
      <div className={`relative rounded-2xl overflow-hidden flex flex-col
        bg-[#1a1a2e] dark:bg-gray-900 border transition-all duration-200
        hover:shadow-xl hover:shadow-indigo-900/30 hover:-translate-y-0.5
        ${isActive ? 'border-gray-700/60' : 'border-gray-700/40 opacity-60'}`}>
        ...
        <div className="relative w-full" style={{ aspectRatio: '4/3' }}>
          ...
          {isPopular && (
            <span className="absolute top-2 left-2 text-[10px] font-bold tracking-wide
              px-2 py-0.5 rounded bg-emerald-500 text-white shadow">
              POPULAR
            </span>
          )}
          ...
          <div className="absolute top-2 right-2" ref={menuRef}>
            ...
            {menuOpen && (
              <div className="absolute right-0 top-8 z-30 min-w-[140px]
                bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden
                animate-[fadeup_0.12s_ease_both]">
                <button
          ...
          {!isActive && (
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
              <span className="text-[11px] font-semibold px-2.5 py-1 rounded-full
                bg-gray-800/90 text-gray-400 border border-gray-700">Inactive</span>
            </div>
          )}
        </div>
