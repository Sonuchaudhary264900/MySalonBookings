...
const CatalogCard = ({ name, onAdd }) => (
  <div className="bg-gray-50 dark:bg-gray-900/60 border border-dashed border-gray-200 dark:border-gray-700
    rounded-2xl p-3 flex flex-col gap-2 hover:border-indigo-300 dark:hover:border-indigo-700
    hover:bg-indigo-50/40 dark:hover:bg-indigo-950/20 transition-all duration-150 group">
    <div className="w-full aspect-square rounded-xl bg-gray-100 dark:bg-gray-800
    ...
    <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 truncate text-center leading-tight">
      {name}
    </p>
    <button
      onClick={onAdd}
      className="w-full py-1.5 rounded-xl text-[11px] font-bold
        ...
        hover:border-indigo-600 transition-all duration-150">
      + Add
    </button>
  </div>
);
        ...
        {allServices.length > 0 && (
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl border text-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-400 transition-colors"
            />
          </div>
        )}
              ...
              {allCategoryLabels
                ...
                .map(cat => {
                  ...
                  const catalogGrid = notAdded.length > 0 ? (
                    <div className={`grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3
                      ${realSvcs.length > 0 ? 'mt-3 pt-3 border-t border-dashed border-gray-100 dark:border-gray-800' : ''}`}>
                      {notAdded.map(cs => (
                        <CatalogCard
                          key={cs.name}
                          name={cs.name}
