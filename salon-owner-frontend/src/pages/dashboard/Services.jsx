  ...
  return (
    <DashboardLayout>
      <div className="space-y-5">
        ...
        {/* ── Level 1: Category Navigation (sticky) ── */}
        {allServices.length > 0 && !loading && (
          <CategoryNav
          ...
          {loading && !allServices.length ? (
            ...
            <div className="space-y-4">
              ...
              <div className="bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 rounded-2xl p-5 flex flex-col sm:flex-row items-center gap-4">
                ...
                <button onClick={() => handleOpenModal(null)}
                  className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold shrink-0 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white shadow-lg shadow-indigo-500/20 transition-all">
                  <Plus className="w-4 h-4" /> Add Service
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center py-16 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 font-medium">
                {search ? `No services match "${search}"` : 'No services in this section'}
              </p>
              <button
                onClick={() => { setSearch(''); setSelectedCatLabel(null); setSelectedSubLabel(null); }}
                  ...
                  <div key={cat} className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
                    {showHeader && (
                      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 dark:border-gray-800">
                        ...
                        <span className="text-xs font-semibold text-gray-400 dark:text-gray-500 px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-800">{svcs.length}</span>
                      </div>
                    )}
                    <div className="p-4">
                      {!showSplit ? renderCards(svcs) : isSalon ? (
