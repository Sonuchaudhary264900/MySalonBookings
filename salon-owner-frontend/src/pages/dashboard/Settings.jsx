...
// ─── Edit Categories helper ───────────────────────────────────
const buildSelectionsFromSalon = (catList, offeredCategories) =>
...
const EditCategoriesContent = ({ salon, updateSalon }) => {
  const [gender,      setGender]      = useState(salon?.servedGender || '');
  const [loading,     setLoading]     = useState(false);
  const [expandedKey, setExpandedKey] = useState(null);

  const [maleSelections,   setMaleSelections]   = useState(() => buildSelectionsFromSalon(MALE_CATEGORIES,   salon?.offeredCategories));
  const [femaleSelections, setFemaleSelections] = useState(() => buildSelectionsFromSalon(FEMALE_CATEGORIES, salon?.offeredCategories));
  const [unisexSelections, setUnisexSelections] = useState(() => buildSelectionsFromSalon(UNISEX_CATEGORIES, salon?.offeredCategories));
  ...
  useEffect(() => {
    ...
    setMaleSelections(buildSelectionsFromSalon(MALE_CATEGORIES,   salon.offeredCategories));
    setFemaleSelections(buildSelectionsFromSalon(FEMALE_CATEGORIES, salon.offeredCategories));
    setUnisexSelections(buildSelectionsFromSalon(UNISEX_CATEGORIES, salon.offeredCategories));
  ...
  }, [salon?.servedGender, salon?.offeredCategories, salon?.kidsHaircut, salon?.atHomeServices]);
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-gray-500">
        Update the service categories and sub-services your salon offers.
      </p>
      ...
      {gender && currentCats.length > 0 && (
        <div className="space-y-2">
          ...
          {/* Optional add-ons for male/female */}
          {gender === 'male' && (
...
const SettingsPage = () => {
  ...
  const SECTIONS = [
    ...
    },
    {
      ...
      subtitle: 'Edit the service categories your salon offers',
      content: <EditCategoriesContent salon={salon} updateSalon={updateSalon} />,
    },
    {
      id: 'notifications',
      icon: Bell,
