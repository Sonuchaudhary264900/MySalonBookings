...
export default function ProfileScreen() {
  ...
  const qrRef = useRef(null);      // ref to the local QR svg (react-native-qrcode-svg)
  const [qrPng, setQrPng] = useState(null); // base64 PNG of the locally-rendered QR
...
function draw(){
  ...
  try{ window.ReactNativeWebView.postMessage(c.toDataURL('image/png').split(',')[1]); }
  catch(e){ window.ReactNativeWebView.postMessage('ERROR'); }
}
var src=${qrSrc};
if(!src){ window.ReactNativeWebView.postMessage('ERROR'); return; }
var img=new Image();
img.onload=function(){ ctx.drawImage(img,110,110,180,180); draw(); };
        ...
        <View style={styles.qrOverlay}>
          <View style={styles.qrBox}>
            ...
            <View style={styles.qrCodeWrap}>
              <QRCode value={qrValue || ' '} size={220} quietZone={6} />
            </View>
            ...
            >
              ...
              <Text style={{ color: '#fff', fontWeight: '700', fontSize: 12 }}>Share Booking Link</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
