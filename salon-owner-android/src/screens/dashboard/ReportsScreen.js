...
export default function ReportsScreen() {
  ...
  const fetchAnalytics = useCallback(async (presetIndex) => {
    ...
    } catch { setAnalytics(null); } finally {
      setLoading(false);
    }
  }, []);
        ...
        >
          ...
          {d?.topServices?.length > 0 && (
            <View style={styles.section}>
              ...
              {d.topServices.map((s, i) => (
                <View key={i} style={styles.topServiceRow}>
                  <View style={styles.rankBadge}>
                    <Text style={styles.rankText}>#{i + 1}</Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.topServiceName}>{s.name || s.serviceName}</Text>
                    <Text style={styles.topServiceSub}>{s.bookings ?? s.count ?? 0} bookings</Text>
                  </View>
            ...
            <View style={styles.section}>
              ...
              {d.dailyRevenue.map((row, i) => (
                <View key={i} style={[styles.tableRow, i % 2 === 0 && { backgroundColor: '#f9fafb' }]}>
                  ...
                  <Text style={[styles.tableCell, { textAlign: 'center', color: '#374151' }]}>
                ...
                return (
                  <View key={b._id} style={styles.bookingRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.bookingMeta}>{b.serviceName} · {formatTime(b.appointmentTime)}</Text>
                    </View>
          ...
          {!d && (
            <View style={{ alignItems: 'center', paddingVertical: 48 }}>
              ...
              <Text style={{ color: '#9ca3af', marginTop: 8 }}>No analytics data available</Text>
            </View>
          )}
        </ScrollView>
      )}
...
const styles = StyleSheet.create({
  ...
  presetChipTextActive: { color: '#4f46e5', fontWeight: '700' },
  ...
  rankText: { fontSize: 12, fontWeight: '700', color: '#4f46e5' },
