...
export default function ReportsScreen() {
  ...
  const shareReport = async () => {
    ...
    if (d.topServices?.length) {
      ...
      d.topServices.slice(0, 5).forEach((s, i) =>
        lines.push(`  ${i + 1}. ${s.name || s.serviceName} — ₹${s.revenue ?? 0} (${s.bookings ?? s.count ?? 0} bookings)`)
      );
    }
    lines.push('', `Generated by SmartSalon`);
    try {
      ...
      <View style={[styles.header, { paddingTop: 12 + insets.top }]}>
        ...
        {analytics && (
          <View style={styles.exportRow}>
            ...
            <TouchableOpacity style={[styles.exportBtn, { backgroundColor: 'rgba(255,255,255,0.25)' }]} onPress={shareReport}>
