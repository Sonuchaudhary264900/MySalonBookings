...
import {
  ...
  Image, SafeAreaView, StatusBar, ActivityIndicator, Platform,
} from 'react-native';
  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: theme.bg }]}>
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {TYPES.map(({ key, label, icon }) => {
            ...
            return (
              ...
              >
                ...
                <Text style={{ fontSize: 12, fontWeight: '600', color: active ? '#8b5cf6' : theme.subText, marginTop: 6 }}>
                  {label}
                </Text>
              </TouchableOpacity>
            );
          ...
          <View style={styles.severityRow}>
            {SEVERITIES.map(({ key, label, color }) => {
              ...
              return (
                ...
                >
                  <Text style={{ fontSize: 12, fontWeight: '600', color: active ? color : theme.subText }}>{label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        {/* Screenshot */}
        <Text style={[styles.label, { color: theme.subText }]}>Screenshot (optional)</Text>
        {screenshot ? (
          ...
          >
            ...
            <Text style={{ fontSize: 13, fontWeight: '500', color: theme.subText, marginLeft: 8 }}>Add a screenshot</Text>
          </TouchableOpacity>
        )}
        ...
        {/* My submissions */}
        <Text style={[styles.sectionTitle, { color: theme.text }]}>My Submissions</Text>
        <View style={[styles.listCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
          {loading ? (
            ...
            <View style={{ padding: 24, alignItems: 'center' }}>
              <Text style={{ fontSize: 13, color: theme.subText }}>No submissions yet</Text>
            </View>
          ) : (
            items.map((item, i) => {
              ...
              return (
                ...
                >
                  <View style={styles.itemTop}>
                    ...
                    <View style={{ flex: 1 }}>
                      <Text style={[styles.itemSubject, { color: theme.text }]} numberOfLines={1}>{item.subject}</Text>
                  ...
                  {item.status === 'resolved' && (
                    <TouchableOpacity onPress={() => handleReopen(item._id)} disabled={reopeningId === item._id} style={styles.reopenBtn}>
                      ...
                      <Text style={{ fontSize: 12, fontWeight: '600', color: '#8b5cf6', marginLeft: 6 }}>
        ...
        {pages > 1 && (
          <View style={styles.pagerRow}>
            ...
            >
              <Text style={{ fontSize: 12, fontWeight: '600', color: theme.subText }}>Previous</Text>
            </TouchableOpacity>
            <Text style={{ fontSize: 12, color: theme.subText }}>Page {page} of {pages}</Text>
            <TouchableOpacity
              onPress={() => loadFeedback(page + 1)}
              disabled={page >= pages}
              style={[styles.pagerBtn, { borderColor: theme.border, backgroundColor: theme.card, opacity: page >= pages ? 0.5 : 1 }]}
            >
              <Text style={{ fontSize: 12, fontWeight: '600', color: theme.subText }}>Next</Text>
            </TouchableOpacity>
          </View>
        )}
      </ScrollView>
