...
// ===================================================
// GOOGLE SIGN-IN LOGIN (existing owners only — registration handled separately)
// ===================================================
exports.googleLogin = async (req, res) => {
  try {
    const { idToken } = req.body;
    if (!idToken) {
      return res.status(400).json(formatErrorResponse('Google ID token is required', 400));
    }
    ...
    try {
      ...
      return res.status(401).json(formatErrorResponse('Invalid or expired Google token.', 401));
    }

    const googleId = payload.sub;
    const email = (payload.email || '').toLowerCase().trim();

    // 1. Lookup by Google ID (fastest — set on all future logins)
    let owner = await Owner.findOne({ googleId }).select('+isBanned');

    // 2. Lookup by verified email and link the Google account
    if (!owner && email && payload.email_verified) {
      owner = await Owner.findOne({ email }).select('+isBanned');
      if (owner && !owner.googleId) owner.googleId = googleId;
    }

    if (!owner) {
      return res.status(404).json(
        formatErrorResponse(`No GlowLoox Partner account found for ${email || 'this Google account'}. Please register to create your account.`, 404)
      );
    }
    ...
    console.error('Error in owner googleLogin:', error);
    res.status(500).json(formatErrorResponse(error.message || messages.GENERIC.ERROR, 500));
  }
};
